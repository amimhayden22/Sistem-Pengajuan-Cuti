...
@section('title') Form Tambah Jabatan @endsection
            ...
            <div class="card-body">
                <form action="{{ route('positions.index') }}" method="POST" class="needs-validation" novalidate="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Contoh: Web Development" required>
                        @if (count($errors) > 0)
                            @error('name')
